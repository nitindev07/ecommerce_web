  ...
  return (
    <div className='flex py-4 gap-x-4 lg:px-6 border-b border-gray-200  w-full font-light text-gray-500'>
      <div  className='w-full min-h-[150px] flex items-center gap-x-4'>
        {/* image */}
        <Link onClick={handleopen} to={`/product/${id}`}>
          <img className='max-w-[80px]' src={image} alt="" />
        </Link>
        <div className='w-full flex flex-col'>
          {/* title and remove button */}
          <div className='flex justify-between mb-2'>
            <Link onClick={handleopen} className='text-sm uppercase font-medium max-w-[240px] text-primary hover:underline' to={`/product/${id}`}>
              {title}
            </Link>
